...
def gps_status(drone):
    ...
    gps_msg = None
    print(this_drone)
    while gps_msg is None:
        try:
            gps_msg   = this_drone.recv_match(type='GPS_RAW_INT ', blocking=True, timeout=0.001)
            print(gps_msg)
            fix_state = gps_msg.fix_type
            altitude  = gps_msg.alt
            latitude  = gps_msg.lat
            longitude = gps_msg.lon
            break
        except:
            print("doesn't recieve message")
            pass
        sleep(0.05)
    location_dict[this_drone.sysid] = (latitude,longitude,altitude)
    local_lock.release()
    #return (str(this_drone.sysid) + "\t" + str(latitude) + "\t" + str(longitude) + "\t" + str(altitude))
    ...
    while True:
        ...
        msg = drone.recv_match(
            type=['MISSION_COUNT', 'MISSION_ITEM_INT', 'MISSION_REQUEST_INT', 'MISSION_REQUEST',
                  'MISSION_ACK', 'GLOBAL_POSITION_INT', 'STATUSTEXT', 'GPS_RAW_INT'], blocking=True, timeout=0.5)
        try:
            if msg.name is 'MISSION_ACK' and msg.type is 0:
                ...
                #print(msg)
                mission_cnt = msg.seq
            if msg.get_type() is 'GPS_RAW_INT':
                print(msg)
            else:
                #print(msg)
                pass
...
def write_location(drones_set):
    lock.acquire()
    if(drones_set):
        while True:
            if os.stat("drone_location.txt").st_size ==0:
                print ("write start")
                f = open("drone_location.txt", 'w')
                f.write("sys_id\tlat\tlon\talt\n")
                for drone in drones_set:
                    get_gps_thread = threading.Thread(target=gps_status, args=(drone,))
                    get_gps_thread.start()
                    get_gps_thread.join()
                for key, value in location_dict.items():
                    f = open("drone_location.txt", 'a')
                    f.write(str(key) + '\t' + value[0] + '\t' + value[1] + '\t' + value[2])
                    f.close()
            else:
                sleep(0.001)
                pass
...
def read_command(drones_set):
    lock = threading.Lock()
    lock.acquire()
    if(drones_set):
        while True:
            if os.stat("drone_commander.txt").st_size !=0:
                print ("command start")
                f = open("drone_commander.txt", 'w')
    lock.release()
...
def make_connection(input_data, var_drones_set):
    failed_data = list()
    for lines in input_data:
        ...
        if ack is None:
            ...
            #comment out until mission upload
    ...
    if len(failed_data) is not 0:
        while True:
            ...
            print(check_retry)
            if check_retry == 'y':
                make_connection(failed_data, var_drones_set)
            elif check_retry == 'n':
                break
            else:
                break
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if rtk_use is "y":
            ...
            print('else')
            break
    make_connection(input_data, var_drones_set)
    writer_thread = threading.Thread(target=write_location, args=[var_drones_set])
    writer_thread.start()
